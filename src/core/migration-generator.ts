...
// Modified generateSpannerCreateTableDDL to separate table creation from index/FK creation
function generateSpannerJustCreateTableDDL(table: TableSnapshot): string {
  ...
  return createTableSql; // Just the CREATE TABLE statement
}
...
function generateSpannerDdl(diffActions: TableDiffAction[]): string[][] {
  const allDdlStatements: string[] = []; // Changed from categorized arrays to a single sequential list
  ...
  for (const action of diffActions) {
    ...
    switch (action.action) {
      case "add": {
        const table = action.table;
        allDdlStatements.push(generateSpannerJustCreateTableDDL(table));
        ...
        // Add FKs after table and its indexes are defined
        for (const columnName in table.columns) {
          const column = table.columns[columnName];
          if (column.references) {
            allDdlStatements.push(
      ...
      case "remove":
        allDdlStatements.push(`DROP TABLE ${currentTableNameEsc}`);
        break;
      case "change": {
  ...
  // The batching logic is now applied to `allDdlStatements` which is populated sequentially per action.
  const finalBatches: string[][] = [];
  let currentBatch: string[] = [];
  let currentBatchContainsValidatingDdl = false;

  for (const ddl of allDdlStatements) {
    // Iterate over the sequentially populated allDdlStatements
