...
export function generateCreateTablePostgres(
  tableConfig: TableConfig<string, any>
): string {
  const tableName = escapeIdentifier(tableConfig._name); // Use _name
  ...
  for (const key in tableConfig) {
    if (
      ...
      // Check if it's a column config (e.g. by checking for a known property like 'dialectTypes')
  ...
  for (const column of actualColumns) {
    ...
    if (column.primaryKey) {
      ...
      if (pkCount === 1 && !tableConfig._compositePrimaryKey) {
    ...
    if (column.references) {
      ...
      if (!referencedTableName) {
        console.warn(
          `Could not determine referenced table name for column "${column.name}" in table "${tableConfig._name}". FK constraint skipped.`
        );
      } else {
  ...
  if (
    ...
    const compositePkCols =
      tableConfig._compositePrimaryKey.columns.map(escapeIdentifier);
    ...
    if (primaryKeyColumns.length > 1) {
      ...
      const pkCol = actualColumns.find((c) => c.name === pkColName);
      if (
        pkCol &&
        !columnsSql.find(
          (s) =>
  ...
  if (tableConfig._indexes) {
    for (const index of tableConfig._indexes) {
      const indexName = index.name
        ? escapeIdentifier(index.name)
        : escapeIdentifier(
            `uq_${tableConfig._name}_${index.columns.join("_")}`
          );
